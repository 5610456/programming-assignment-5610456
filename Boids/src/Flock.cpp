...
//     // Make sure your shader MVP uniform includes view & projection in your NGLScene
...
// Move all boids by delta amounts (the method NGLScene expects)
void Flock::moveFlock(float _dx, float _dy, float _dz)
{
  move(_dx, _dy, _dz); // Update flock center position as well
  for (auto &b : m_boids)
  {
    b.position.m_x += _dx;
    b.position.m_y += _dy;
...
{
  // Save _value to a member variable that controls alignment behavior
